...
std::uint32_t client::run_transactions(std::uint8_t thread_count) {
	...
	for(std::uint32_t t = 0; t < thread_count; t++) {
		threads[t] = std::thread(do_transactions_concurrent_timed, &hash_index, std::ref(wl), thread_operation_count, std::ref(timings[t]));
		utils::stick_thread_to_core(threads[t].native_handle(), (t*2)+1);
	}
	for(std::uint32_t t = 0; t < thread_count; t++) {
		threads[t].join();
...
std::uint32_t client::run_workload() {
	...
	// Insertions 	- Initialization of the index
	for(std::uint32_t t = 0; t < thread_count; t++) {
		threads[t] = std::thread(do_insertions_concurrent_timed, &hash_index, std::ref(wl), thread_record_count, std::ref(timings[t]));
		utils::stick_thread_to_core(threads[t].native_handle(), (t*2)+1);
	}
	for(std::uint32_t t = 0; t < thread_count; t++) {
		threads[t].join();
